...
local function GeoFileCompare(datfile, sumfile)
    local rt = sys.exec('if [ "$(sha256sum ' .. datfile .. ' | awk \'{print $1}\')" = "$(cat ' .. sumfile .. ' | awk \'{print $1}\')" ]; then echo -n 0; else echo -n 1; fi')
    if rt == "1" then
        return 1
    elseif rt == "0" then
        return 0
